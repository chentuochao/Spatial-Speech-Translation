...
@register_quality_scorer("ASR_BLEU_INDIVIDUAL")
class ASRSacreBLEUScorer(QualityScorer):
    """
    ASR + SacreBLEU Scorer (BETA version)
    ...
    def __call__(self, instances: Dict) -> float:
        ...
        ref_tag = [ins.reference for ins in instances.values()]
        bleu_score = BLEU(tokenize=self.tokenizer)
        score_list = []
        for i in range(len(transcripts)):
            score = bleu_score.corpus_score([transcripts[i]], [[ref_tag[i]]]).score
            score_list.append(score)
        output_path = Path(instances[0].prediction).absolute().parent.parent
        file_path = output_path / "score_list.txt"
