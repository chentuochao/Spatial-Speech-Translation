...
class StreamSpeechS2STAgent(SpeechToSpeechAgent):
    ...
    def load_model_vocab(self, args):
        ...
        print("chunk size", chunk_size, args.source_segment_size)
        ...
        print("self.dict tgt", len(self.dict["tgt"]))
        for k, v in task.multitask_tasks.items():
            self.dict[k] = v.tgt_dict

    @torch.inference_mode()
    def policy(self):
        # print("Step1 input source = ", len(self.states.source)/48000)  # T
        t0 = time.time()
        feature = self.feature_extractor(self.states.source)
        # print("step1 time =", time.time() - t0)
        ...
        # print("Step2 input features = ", src_indices.shape, src_lengths)  # 1 X T x E
        t0 = time.time()
        self.encoder_outs = self.generator.model.forward_encoder(
            {"src_tokens": src_indices, "src_lengths": src_lengths}
        )
        # print("step2 time =", time.time() - t0)
        # print("Step3 asr_ctc_generator input = ", len(self.encoder_outs), self.encoder_outs[0]["encoder_out"][0].shape) # T//4 x 1 x E
        t0 = time.time()
        finalized_asr = self.asr_ctc_generator.generate(
            self.encoder_outs[0], aux_task_name="source_unigram"
        )
        # print("step3 time =", time.time() - t0)
        ...
        # 1. MT decoder
        # print("Step4 generator_mt input = ", self.encoder_outs[0]["encoder_out"][0].shape, src_indices.shape) # T//4 x 1 x E
        t0 = time.time()
        ...
        # print("Step5 mt_decoder input = ", self.encoder_outs[0]["encoder_out"][0].shape, prev_output_tokens_mt) # T//4 x 1 x E
        t0 = time.time()
        ...
        print("Target dict - ", self.ctc_generator.tgt_dict.blank_index, self.generator.eos, self.ctc_generator.tgt_dict.pad_index, self.ctc_generator.tgt_dict.bos_index, self.ctc_generator.tgt_dict.eos_index)
