...
class OnlineFeatureExtractor:
    ...
    def __call__(self, new_samples, sr=ORG_SAMPLE_RATE):
        ...
        sample = torch.tensor([samples])

        ## if it is binural output use the firt channel
        if len(sample.shape) == 3:
            sample = sample[:, :, 0]
        waveform, sample_rate = convert_waveform(
            sample, sr, to_mono=True, to_sample_rate=16000
        )
        output = extract_fbank_features(waveform, 16000)
        output = self.transform(output)
        return torch.tensor(output, device=self.device), torch.tensor(waveform, device=self.device)
...
class StreamSpeechS2STAgent(SpeechToSpeechAgent):
    ...
    def __init__(self, args):
        super().__init__(args)
        # self.waveform, sample_rate = torchaudio.load("/gscratch/intelligentsystems/tuochao/Spatial-StreamSpeech/StreamSpeech/example/clean.wav")
        ...
        self.seamless_dur = args.seamless_dur
        ...
        ### load seamless vocoder 
        self.tgt_lang = "eng"
        vocoder_name = "vocoder_pretssel_16khz"
        self.vocoder = load_pretssel_vocoder_model(vocoder_name, device=self.device, dtype=torch.float16)
        self.vocoder.eval()
        assert isinstance(self.vocoder, PretsselVocoder)
        _gcmvn_mean, _gcmvn_std = load_gcmvn_stats(vocoder_name)
        self.gcmvn_mean = torch.tensor(
            _gcmvn_mean, device=self.device, dtype=torch.float16
        )
        self.gcmvn_std = torch.tensor(_gcmvn_std, device=self.device, dtype=torch.float16)

        self.dur_prediction = args.dur_prediction
        self.silero_vad = args.silero_vad
        if self.silero_vad:
            self.voice_detector = load_silero_vad()
            if self.device == "cuda":
                self.voice_detector = self.voice_detector.cuda()
        if self.dur_prediction:
            from agent.unit_dur.duration_prediction import build_model2
            self.dur_predictor = build_model2(self.vocoder.encoder_frontend.embed_tokens, use_rnn = False)
            checkpoint = torch.load("agent/unit_dur/dur_predictor.pt")
            self.dur_predictor.load_state_dict(checkpoint['model_state_dict'])
            if self.device == "cuda":
                self.dur_predictor = self.dur_predictor.cuda()
    ...
    def add_args(parser):
        ...
        parser.add_argument(
            "--seamless-dur", type=int, default=2, help="seamless_duration scale"
        )
        ...
        parser.add_argument(
            ...
            type=bool,
            default=False,
            help="whether apply voice detecion",
        )
    ...
    def load_model_vocab(self, args):
        ...
        for model in self.models:
            model.eval()
            model.share_memory()
            # print("*****DEBUG, speech_to_speech.streamspeech.agent.py line 402", self.gpu, torch.cuda.is_available())
            if self.gpu:
                model.cuda()
    ...
    def pretssel_vocoder(self, unit, source):
        ## wehther apply voice detector
        # source = self.waveform.to(source.device)  
        # print("source!!!!", len(source), type(source), self.waveform.shape)
        # print("source!!!!", source.shape)
        if self.silero_vad:
            time_stamps = get_speech_timestamps(source, self.voice_detector)
            new_source = []
            for stamp in time_stamps:
                new_source.append(source[stamp['start']:stamp['end']])
            # print("voice_detector: ", time_stamps)
            if len(new_source) > 0:
                source = torch.cat(new_source, dim = 0)
            
            # sf.write(f"./debug/newsource{self.accum}.wav", new_source.float().cpu().numpy(), samplerate=16000)
            # sf.write(f"./debug/oldource{self.accum}.wav", source.float().cpu().numpy(), samplerate=16000)
            

        def gcmvn_normalize(seqs: torch.Tensor, gcmvn_mean, gcmvn_std) -> torch.Tensor:
            result: torch.Tensor = seqs.subtract(gcmvn_mean).divide(gcmvn_std)
            return result
        ...
        if self.dur_prediction:
            # print(unit.shape)
            duration = self.dur_predictor(unit.unsqueeze(0))
            duration = torch.clamp(
                torch.round(duration).long(), min=1, max = 5
            ).squeeze(0)

        # print(duration, unit.shape)
        duration *= self.seamless_dur
        ...
        audio_dict: WaveformToFbankInput = {
            "waveform": torch.tensor(
                source, dtype=torch.float32, device=unit.device
            ).unsqueeze(1),
            "sample_rate": 16000,
        }
        feats = convert_to_fbank(audio_dict)["fbank"]
        feats = gcmvn_normalize(feats, self.gcmvn_mean, self.gcmvn_std)
    ...
    def policy(self):
        feature, raw_wav = self.feature_extractor(self.states.source)
        ...
        for i, hypo in enumerate(finalized_asr):
            ...
            text = text.replace("<s>", "")
            text = text.replace("</s>", "")
            print("*****DEBUG speech_to_speech.streamspeech.agent.py line 454", text)
        ...
        for i, hypo in enumerate(finalized_mt):
            ...
            text = text.replace("<s>", "")
            text = text.replace("</s>", "")
            print("*****DEBUG speech_to_speech.streamspeech.agent.py line 603", text)
            if len(text) > 0 and text[0] == " ":
                text = text[1:]
            if self.states.source_finished and not self.quiet:
        ...
        # print("Target dict - ", len(self.ctc_generator.tgt_dict), len(self.generator.tgt_dict), self.ctc_generator.tgt_dict.blank_index, self.generator.eos, self.ctc_generator.tgt_dict.pad_index, self.ctc_generator.tgt_dict.bos_index, self.ctc_generator.tgt_dict.eos_index)
        ...
        wav, dur = self.pretssel_vocoder(x["code"], raw_wav[0])
        # print(type(wav), type(dur))
        # cur_unit = np.array(cur_unit)
        all_wav_length = dur.sum() * 160
        cur_wav_length = dur[:, -len(cur_unit) :].sum() * 160
        new_wav = wav[-cur_wav_length:]
        # print("cur_wav_length = ", cur_wav_length, all_wav_length, wav.shape)
        ...
        ### save for debug
        # sf.write(f"res/debug/debug{self.accum}.wav", wav.float().cpu().numpy(), samplerate=16000)
        # self.accum += 1
        ...
        return WriteAction(
            SpeechSegment(
                content=new_wav.tolist(), #new wave
                sample_rate=SAMPLE_RATE,
