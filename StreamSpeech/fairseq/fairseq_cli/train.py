...
def freeze_param(model, freeze_module):
    
    freeze_list = freeze_module.split(' ')
    print("apply param freezing", freeze_list)

    if "encoder" in freeze_list:
        model.freeze_encoder = True
    def print_params_per_attribute(model):
        for name, module in model.named_children():
            num_params = sum(p.numel() for p in module.parameters())
            print(f"{name}: {num_params/1e6} M parameters")
    ...
    def count_params(model):
        ...
        print(f"Number of frozen parameters: {frozen_params/1e6} M")
    print_params_per_attribute(model)
    ...
    for name, module in model.named_children():
        if name in freeze_list:
            ...
            if name == "encoder":
                for name0, mod in module.named_modules():
                    if "batch_norm" in name0 and isinstance(mod, nn.BatchNorm1d):
                        print("freeze eval() batchnorm =", name0, mod)
                        mod.eval()
    count_params(model)
...
def main(cfg: FairseqConfig) -> None:
    ...
    if cfg.distributed_training.ddp_backend == "fully_sharded":
        ...
        print("*****DEBUG StreamSpeech/fairseq/fairseq_cli/train.py", cfg.model)
        model = task.build_model(cfg.model)
    if cfg.checkpoint.freeze_module is not None:
        model = freeze_param(model,cfg.checkpoint.freeze_module )
    ...
    print("checkpoints-config - ", cfg.checkpoint)
    ...
    while epoch_itr.next_epoch_idx <= max_epoch:
        ...
        epoch_itr = trainer.get_train_iterator(
            ...
            load_dataset=task.has_sharded_data("train"), #false
            # don't cache epoch iterators for sharded datasets
            disable_iterator_cache=task.has_sharded_data("train"),
        )
    train_meter.stop()
...
def train(
    ...
    #     logger.info("validate pretrainwed model on finetune dataset")
    #     valid_losses = validate(cfg, trainer, task, epoch_itr, valid_subsets)
    #     print("valid_losses", valid_losses)
        # logger.info(f"validate pretrainwed model on finetune dataset{valid_losses}")
