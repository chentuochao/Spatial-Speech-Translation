...
def extract_fbank_features_2channel(
    waveform_list: List[torch.FloatTensor],
    sample_rate: int,
    ...
    if output_path is not None and output_path.is_file() and not overwrite:
        print("*****DEBUG whether I will exit")
        print(output_path)
    ...
    for waveform in waveform_list:
        ...
        features = _get_kaldi_fbank(_waveform, sample_rate, n_mel_bins) ## this is not applied torch version is applied
        # print("*****DEBUG StreamSpeech/fairseq/examples/speech_to_text/data_utils.py line 89", features)
        if features is None:
            features = _get_torchaudio_fbank(_waveform, sample_rate, n_mel_bins) # torch version is applied
        if features is None:
            raise ImportError(
                "Please install pyKaldi or torchaudio to enable fbank feature extraction"
            )
        result_features.append(features)
    features = np.concatenate((result_features[0], result_features[1]), axis=1)
...
def IPD_ONNX(real1, imag1, real2, imag2, norm, norm_ref, tol = 1e-6):
    IPD_cos = (real1 * real2 + imag1 * imag2) / (norm * norm_ref + tol) # FT
    IPD_sin = (real2 * imag1 - imag2 * real1) / (norm * norm_ref + tol) # FT

    return IPD_cos, IPD_sin


def MC_features_ONNX(r1, i1, r2, i2, eps=1e-6):
    # Input: F, T
    # Compute magnitude
    norm1 = torch.sqrt(torch.square(r1) + torch.square(i1))
    norm2 = torch.sqrt(torch.square(r2) + torch.square(i2))

    # Compute ILD
    ILD_m = torch.log10(torch.div(norm2 + eps, norm1 + eps))

    # Compute IPD
    IPD_cos, IPD_sin = IPD_ONNX(r1, i1, r2, i2, norm2, norm1)

    out = torch.stack([r1, i1, r2, i2, ILD_m, IPD_cos, IPD_sin], dim=0) # [B, 3M-3, f, T]
    
    return out

def extract_stft_features(waveform_list, sample_rate, window_ms = 25, hop_size = 10):
    ## input binural audio - [audio1, audio2]
    ## only support 2-channel
    window_size = int(sample_rate * window_ms/1000) # 25ms
    hop_size = int(sample_rate * window_ms/1000) # 10ms
    raw = torch.stack(waveform_list, dim = 0)
    # x = 2xT
    # stft_x = 2 x F x T x 2
    stft_x = torch.stft(raw, n_fft = window_size, hop_length = hop_size,
                    win_length = window_size, normalized=False, return_complex=False)

    xl = stft_x[0] # F x T x 2
    xr = stft_x[1] # F x T x 2

    stft_feat = MC_features_ONNX(r1 = xl[:,:,0], i1 = xl[:,:,1], r2 = xl[:,:,0], i2 = xl[:,:,1]) 
    M , F , T = stft_feat.shape
    stft_feat = stft_feat.reshape((M*F, T))
    stft_feat = stft_feat.transpose((0, 1))
    return stft_feat.numpy() # T x (Mx F)


def extract_fbank_features_2channel_phase(
    waveform_list: List[torch.FloatTensor],
    sample_rate: int,
    output_path: Optional[Path] = None,
    n_mel_bins: int = 80,
    ...
    ### add stft phase features
    stft_feat = extract_stft_features(waveform_list, sample_rate)
    ...
    features = np.concatenate((result_features[0], result_features[1], stft_feat), axis=1) # T, F
...
def extract_fbank_features(
    ...
    features = _get_kaldi_fbank(_waveform, sample_rate, n_mel_bins)
    if features is None:
        # print("apply _get_torchaudio_fbank")
        features = _get_torchaudio_fbank(_waveform, sample_rate, n_mel_bins)
    if features is None:
        raise ImportError(
            "Please install pyKaldi or torchaudio to enable fbank feature extraction"
