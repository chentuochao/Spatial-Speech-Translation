...
def check_grad_nan(model):
    for param in model.parameters():
        if torch.isnan(param.grad).any():
            print(f"NaN detected in gradients at epoch, reverting to last valid model state.")
            nan_detected = True
            return True
    return False
    ...
    def load_checkpoint(
        ...
        if bexists:
            ...
            try:
                ...
                if self.cfg.checkpoint.load_partial is None:
                    ...
                    for k,v in state["model"].items() :
                        load_list = self.cfg.checkpoint.load_partial.split(' ')
                        need_load = False
                        for m in load_list:
                            if k.startswith(m):
                                need_load = True
                    ...
                    self.model.load_state_dict(
                        filtered_state_dict, strict=False, model_cfg=self.cfg.model
                    )
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    if check_for_nan(self.model):
                        print("[Error!!!!] model params have nan!!!")
                    loss, sample_size_i, logging_output = self.task.train_step( # <ctc_unity.tasks.speech_to_speech_ctc.SpeechToSpeechCTCTask
                        sample=sample,
                        model=self.model,
                    ...
                    # print("*****DEBUG fairseq/fairseq/trainer.py model has nan param?", check_for_nan(self.model))
