...
class FairseqTask(object):
    ...
    def train_step(
        ...
        ### freeze the batch norm when freezing the S2T module
        if getattr(model, "freeze_encoder", False): #model.freeze_encoder:            
            for name0, mod in model.named_modules():
                if "encoder" in name0 and "batch_norm" in name0 and isinstance(mod, torch.nn.BatchNorm1d):
                    mod.eval()
