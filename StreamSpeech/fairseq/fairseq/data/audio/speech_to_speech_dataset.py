...
class SpeechToSpeechDatasetCreator(object):
    ...
    KEY_TGT_AUDIO_RAW, KEY_TGT_N_FRAMES_RAW = "tgt_audio_raw", "tgt_n_frames_raw"
    # optional columns
    ...
    def _from_list(
        ...
        if unit_repeat:
            tgt_audio_paths = [
                s[cls.KEY_TGT_AUDIO_RAW]
                if target_is_code
                else (audio_root / s[cls.KEY_TGT_AUDIO_RAW]).as_posix()
                for s in samples
            ]
        else:
            tgt_audio_paths = [
        ...
        if unit_repeat:
            tgt_n_frames = [int(s[cls.KEY_TGT_N_FRAMES_RAW]) for s in samples]
        else:
