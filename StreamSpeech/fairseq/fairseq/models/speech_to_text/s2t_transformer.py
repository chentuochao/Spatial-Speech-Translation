...
class S2TTransformerEncoder(FairseqEncoder):
    ...
    def __init__(self, args):
        super().__init__(None)
        self.stft_block = args.add_stft
        self.stft_dim = args.stft_dim
        self.mel_dim = args.input_feat_per_channel * args.input_channels
        ...
        print("*****DEBUG StreamSpeech/fairseq/fairseq/models/speech_to_text/s2t_transformer.py line 314", self.conv_version)
        ...
        if self.stft_block:
            self.sep_module = self.subsample = Conv1dSubsampler(
                self.stft_dim, 
                args.conv_channels,
                args.encoder_embed_dim,
                [int(k) for k in args.conv_kernel_sizes.split(",")],
    ...
    def _forward(self, src_tokens, src_lengths, return_all_hiddens=False):
        # src_tokens - B T E
        if self.stft_block:
            src_mel  = src_tokens[:, :, :self.mel_dim]
            x, input_lengths = self.subsample(src_mel, src_lengths)

            src_stft = src_tokens[:, :, self.mel_dim:]
            mask, _ = self.sep_module(src_stft, src_lengths)
            x = x*mask
