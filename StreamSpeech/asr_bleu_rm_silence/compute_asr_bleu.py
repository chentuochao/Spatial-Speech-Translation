...
def compose_eval_data(
    ...
    reference_sentences = reference_sentences[:len(predicted_audio_fp_list)]
...
def run_asr_bleu(args):
    ...
    score_list = []
    for i in range(len(prediction_transcripts)):
        score = sacrebleu.corpus_bleu([prediction_transcripts[i]], [[references[i]]]).score
        score_list.append(score)


    print("individual mnean", np.mean(score_list)) 

    print(bleu_score)

    return prediction_transcripts, bleu_score, score_list
...
def main():
    ...
    prediction_transcripts, bleu_score, score_list = run_asr_bleu(args)
    ...
    file_path = Path(args.results_dirpath) / "score_list.txt"
