...
@dataclass
class SpeechToText2passCTCStreamSpeechCriterionConfig(
    RdropLabelSmoothedCrossEntropyCriterionConfig
):
    k1: int = field(
        default=3,
...
@register_criterion(
    "speech_to_unit_text_2pass_ctc_asr_st",
    dataclass=SpeechToText2passCTCStreamSpeechCriterionConfig,
)
...
class SpeechToUnit2passCTCASRSTMultitaskTaskCriterion(
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # does not compute loss of units output
        # loss, nll_loss, rdrop_kl_loss = 0, 0, 0
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        # print("***DEBUG**** before ", lprobs.shape, target.shape, self.blank_idx, self.eos_idx, self.pad_idx)
        ...
        # if target.size(1) > lprobs.size(0):
        ...
        # print("***DEBUG**** after", lprobs.shape, target.shape, self.blank_idx, self.eos_idx, self.pad_idx)
