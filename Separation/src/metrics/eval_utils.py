...
def itd_diff_torch(s_est, s_gt, sr):
    """
    Computes the ITD error between model estimate and ground truth
    input: (*, 2, T), (*, 2, T)
    """
    s_est = s_est.cpu().numpy()
    s_gt = s_gt.cpu().numpy()

    TMAX = int(round(1e-3 * sr))

    itd_err = []
    for b in range(s_gt.shape[0]):
        itd_est = compute_itd(s_est[b, 0, :], s_est[b, 1, :], sr, TMAX)
        itd_gt = compute_itd(s_gt[b, 0, :], s_gt[b, 1, :], sr, TMAX)
        err = np.abs(itd_est - itd_gt)
        itd_err.append(err)
    
    err = torch.tensor(itd_err).unsqueeze(1)
...
def ild_diff_torch(s_est, s_gt):
    ...
    err = np.abs(ild_est - ild_gt)
    return torch.from_numpy(err).unsqueeze(1)
